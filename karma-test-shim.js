...
function createPathRecords(pathsMapping, appPath) {
  ...
  var moduleName =
      './' + pathParts.slice(Math.max(pathParts.length - 2, 1)).join('/');
...
function onlyBuiltFiles(filePath) { return /^\/base\/temp\//.test(filePath); }

function onlySpecFiles(path) { return /\.spec\.js$/.test(path); }
