...
gulp.task('umd', function(cb) {
  ...
  webpack({
    entry : './temp/index.js',
    output : {
      filename : 'dist/bundles/ng-toggle-switch.js',
      library : 'ng-toggle-switch',
      libraryTarget : 'umd'
    },
    devtool : 'source-map',
    externals : [
...
function startKarmaServer(isTddMode, isSaucelabs, done) {
  ...
  if (isSaucelabs) {
    ...
    config['browsers'] = [
      'SL_CHROME', 'SL_FIREFOX', 'SL_IE10', 'SL_IE11', 'SL_EDGE14', 'SL_EDGE15',
      'SL_SAFARI10', 'SL_SAFARI11'
    ];
...
gulp.task('lint', function() {
  return gulp.src([ PATHS.src, PATHS.demo, '!demo/src/api-docs.ts' ])
      .pipe(tslint(
          {configuration : require('./tslint.json'), formatter : 'prose'}))
...
gulp.task('check-format', function() {
  return doCheckFormat().on('warning', function(e) {
    console.log(
        "NOTE: this will be promoted to an ERROR in the continuous build");
  });
});
...
gulp.task('enforce-format', function() {
  return doCheckFormat().on('warning', function(e) {
    ...
    console.log(
        "See https://github.com/trickeyone/ng-toggle-switch/blob/master/DEVELOPER.md#clang-format");
    process.exit(1);
  });
});
...
function doCheckFormat() {
  return gulp
      .src([
        'gulpfile.js', 'karma-test-shim.js', 'misc/api-doc.js',
        'misc/api-doc.spec.js', 'misc/demo-gen.js', PATHS.src
      ])
...
gulp.task('generate-docs', function() {
  ...
  var docs =
      `const API_DOCS = ${JSON.stringify(getApiDocs(), null, 2)};\n\nexport default API_DOCS;`;

  return gulpFile('api-docs.ts', docs, {src : true})
...
gulp.task('generate-plunks', function() {
  ...
  demoGenUtils.getDemoComponentNames().forEach(function(componentName) {
    plunks =
        plunks.concat(demoGenUtils.getDemoNames(componentName)
                          .reduce(function(soFar, demoName) {
                            soFar.push({
  ...
  return gulpFile(plunks, {src : true})
...
gulp.task(
    ...
    shell.task(
        [ 'webpack --config webpack.demo.js --progress --profile --bail' ],
        {env : {MODE : 'build'}}));
...
gulp.task('demo-push', function() {
  return gulp.src(PATHS.demoDist).pipe(ghPages({
    remoteUrl : "https://github.com/trickeyone/ng-toggle-switch.github.io.git",
    branch : "master"
  }));
});
...
// Public Tasks
gulp.task('clean',
          [ 'clean:build', 'clean:tests', 'clean:demo', 'clean:demo-cache' ]);

gulp.task('build', function(done) {
  runSequence('lint', 'enforce-format', 'ddescribe-iit', 'test', 'clean:build',
              'ngc', 'umd', 'npm', done);
});

gulp.task('deploy-demo', function(done) {
  runSequence('clean:demo', 'build:demo', 'demo-push', 'clean:demo-cache',
              done);
});
