...
describe('APIDocVisitor', function() {
  ...
  it('should extract basic info from directives and components', function() {
    ...
    expect(docs.Foo.fileName)
        .toBe('misc/api-doc-test-cases/directives-no-in-out.ts');
  ...
  it('should extract inputs info', function() {
    var inputDocs = apiDoc([
                      './misc/api-doc-test-cases/directives-with-inputs.ts'
                    ]).Foo.inputs;
  ...
  it('should extract input default value', function() {
    var inputDocs =
        apiDoc([
          './misc/api-doc-test-cases/directives-with-inputs-default-vals.ts'
        ]).Foo.inputs;
  ...
  it('should extract inferred types', function() {
    var inputDocs =
        apiDoc([
          './misc/api-doc-test-cases/directives-with-inputs-types-to-infer.ts'
        ]).Foo.inputs;
  ...
  it('should extract outputs info', function() {
    var outDocs = apiDoc([
                    './misc/api-doc-test-cases/directives-with-outputs.ts'
                  ]).Foo.outputs;
  ...
  it('should extract public methods info', function() {
    var methodDocs = apiDoc([
                       './misc/api-doc-test-cases/directives-with-methods.ts'
                     ]).Foo.methods;
  ...
  it('should not extract public methods info when annotated with @internal',
     function() {
       var methodDocs =
           apiDoc([
             './misc/api-doc-test-cases/component-with-internal-methods.ts'
           ]).Foo.methods;
  ...
  it('should extract documentation from services', function() {
    var serviceDocs = apiDoc([
                        './misc/api-doc-test-cases/services-with-methods.ts'
                      ]).ModalService;

    expect(serviceDocs.fileName)
        .toBe('./misc/api-doc-test-cases/services-with-methods.ts');
    ...
    expect(serviceDocs.methods[1].description)
        .toBe('Checks if a modal is open');
  ...
  it('should extract documentation of properties from services', function() {
    var serviceDocs = apiDoc([
                        './misc/api-doc-test-cases/services-with-properties.ts'
                      ]).ProgressbarConfig;

    expect(serviceDocs.properties.length).toBe(3);

    expect(serviceDocs.properties[0].name).toBe('foo');
    expect(serviceDocs.properties[0].description)
        .toBe('Voluntarily left without a default value.');
    ...
    expect(serviceDocs.properties[1].description)
        .toBe('Maximal value to be displayed in the progressbar.');
  ...
  it('should extract documentation from interfaces', function() {
    var interfaceDocs =
        apiDoc([
          './misc/api-doc-test-cases/interface-with-properties.ts'
        ]).NgbModalOptions;

    expect(interfaceDocs.className).toBe('NgbModalOptions');
    expect(interfaceDocs.description)
        .toBe('Represent options available when opening new modal windows.');
    ...
    expect(interfaceDocs.properties[2].description)
        .toBe('Size of a new modal window.');
  ...
  it('should extract method documentation from interfaces', function() {
    var interfaceDocs = apiDoc([
                          './misc/api-doc-test-cases/interface-with-methods.ts'
                        ]).SomeInterface;
  ...
  it('should extract documentation from documented classes', function() {
    var classDocs =
        apiDoc([ './misc/api-doc-test-cases/class-with-doc.ts' ]).DocumentedFoo;
